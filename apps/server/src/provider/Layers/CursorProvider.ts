...
const CURSOR_ACP_MODEL_DISCOVERY_FAILED_MESSAGE = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart T4 Code, and try again.",
    ...
    if (!cursorSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Cursor is disabled in T4 Code settings.",
        },
      });
    }
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart T4 Code.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
