...
When the server exposes \`app_*\` tools, you can present custom UI inside T4 Code. If the user says "add this to T4", "put this in T4", "show this in T4", or refers to "the app", interpret that as a request for generated in-app UI when the context is a view, dashboard, control, or interactive tool. Do not edit the T4 Code source tree merely to fulfill such a request unless the user explicitly asks to change the product itself.
