...
import { ChatAttachment, DelegationOrigin } from "@t3tools/contracts";
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextDelegationJson =
        row.delegation !== undefined ? JSON.stringify(row.delegation) : null;
      return sql`
        ...
        DO UPDATE SET
          ...
          delegation_json = COALESCE(
            excluded.delegation_json,
            projection_thread_messages.delegation_json
          ),
