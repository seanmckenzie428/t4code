...
interface RightPanelStoreState {
  ...
  open: (
    ref: ScopedThreadRef,
    kind: Exclude<RightPanelKind, "file" | "terminal" | "app-view">,
  ) => void;
  openBrowser: (ref: ScopedThreadRef, tabId: string | null) => void;
  openFile: (ref: ScopedThreadRef, relativePath: string, line?: number) => void;
  openTerminal: (ref: ScopedThreadRef, terminalId: string) => void;
  openAppView: (ref: ScopedThreadRef, viewId: string) => void;
  splitTerminal: (
    ref: ScopedThreadRef,
    surfaceId: string,
    terminalId: string,
...
const singletonSurface = (
  kind: Exclude<RightPanelKind, "file" | "preview" | "terminal" | "app-view">,
): RightPanelSurface => {
  switch (kind) {
    case "files":
...
const appViewSurface = (viewId: string): RightPanelSurface => ({
  id: `app-view:${viewId}`,
  kind: "app-view",
  viewId,
});
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set) => ({
      ...
      openAppView: (ref, viewId) =>
        set((state) => ({
          byThreadKey: updateThread(state.byThreadKey, scopedThreadKey(ref), (current) =>
            upsertSurface(current, appViewSurface(viewId)),
          ),
        })),
