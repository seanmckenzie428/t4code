...
description: Stream an explicit iOS Simulator through pinned serve-sim into the T4 Code in-app browser or another agent browser. Use on Apple Silicon macOS when the user should watch simulator verification live or when browser-visible simulator evidence is needed.
---
...
When running inside T4 Code, use its product-native browser MCP to open the stream. Other agent hosts may use their own browser or preview surface.
