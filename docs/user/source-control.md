...
T4 Code connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

T4 Code works with the platforms your team already uses:
...
- T4 Code can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running T4 Code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in T4 Code and verify GitHub shows as authenticated
...
If both are set, the access token wins. Restart T4 Code and verify the connection in **Source
Control settings**.
...
**Git is required** – T4 Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running T4 Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
